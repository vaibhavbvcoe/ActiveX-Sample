...
void CxDib::CreateFromHANDLE(HANDLE hMem)
{
	if (hDib) {free(hDib); hDib=NULL;}
	...
	if (lpVoid){
		...
		memcpy(&m_bi,pHead,sizeof(BITMAPINFOHEADER));
		//create the image
		if(!Create(m_bi.biWidth,m_bi.biHeight,m_bi.biBitCount)){
			GlobalUnlock(lpVoid);
			return;
		}
		//preserve DPI
		//SetXDPI((long)floor(m_bi.biXPelsPerMeter * 254.0 / 10000.0 + 0.5));
		//SetYDPI((long)floor(m_bi.biYPelsPerMeter * 254.0 / 10000.0 + 0.5));
		//copy the pixels
		if(pHead->biCompression != BI_RGB){
			// BITFIELD case
			// set the internal header in the dib
			memcpy(hDib,&m_bi,sizeof(m_bi));
...
void CxDib::Bitfield2RGB(BYTE *src, WORD redmask, WORD greenmask, WORD bluemask, BYTE bpp)
{
	switch (bpp){
	case 16:
	{
		...
		long effwidth2=(((m_bi.biWidth + 1) / 2) * 4);
		WORD w;
		long y2,y3,x2,x3;
		BYTE *p=GetBits();
		// scan the buffer in reverse direction to avoid reallocations
		for (long y=m_bi.biHeight-1; y>=0; y--){
			y2=effwidth2*y;
			y3=m_LineWidth*y;
			for (long x=m_bi.biWidth-1; x>=0; x--){
				x2 = 2*x+y2;
				x3 = 3*x+y3;
...
void CxDib::CreateFromHBITMAP(HBITMAP hbmp)
{
	if (hDib) {free(hDib); hDib=NULL;}
	...
	if (hbmp) { 
        ...
        HDC dc = ::GetDC(NULL);
		// copy the pixels
        if (GetDIBits(dc, hbmp, 0, m_bi.biHeight, GetBits(),
			(LPBITMAPINFO)hDib, DIB_RGB_COLORS) == 0){ //replace &m_bi with hDib <Wil Stark>
